...
object SetSessionTapir extends App with StrictLogging {
  ...
  def mySetSession: PartialServerEndpointWithSecurityOutput[(UsernamePassword, Seq[Option[String]]), Option[MyScalaSession], Unit, Unit, Seq[Option[String]], Unit, Any, Future] =
  ...
  val login: ServerEndpoint[Any, Future] =
    setNewCsrfToken(checkHeader) {
      ...
      .in("do_login")
      .out(stringBody)
      .serverLogicSuccess(maybeSession => _ =>
        Future.successful("Hello " + maybeSession.map(_.username).getOrElse(""))
      )
