...
[`pekko-http`](https://doc.org.apache.pekko.io/docs/akka-http/current/index.html) is an pekko 
module, which is pre [BSL](https://www.lightbend.com/akka/license) fork of [akka-http](https://doc.akka.io/docs/akka-http/10.2/index.html) and 
originating from and [spray.io](http://spray.io), 
for building *reactive* REST services with an elegant 
DSL.
...
Akka-http-session will compute and include `jti` claim if `pekko.http.session.jwt.include-jti` is set to `true` (it's disabled by default).
...
When updating from a version before 0.5.3, set `pekko.http.session.token-migration.v0-5-3.enabled = true`.

When updating from a version before 0.5.2, set `pekko.http.session.token-migration.v0-5-2.enabled = true`.
